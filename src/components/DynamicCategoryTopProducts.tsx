...
import { ChevronRight, ChevronUp, ChevronDown, ArrowRight, TrendingUp } from "lucide-react";
...
const ITEM_HEIGHT = 88; // approximate height of each product row
...
const CategoryCard = ({
  ...
  const scrollVertical = (dir: 'up' | 'down') => {
    scrollRef.current?.scrollBy({ top: dir === 'up' ? -ITEM_HEIGHT : ITEM_HEIGHT, behavior: 'smooth' });
  };

  const needsScroll = catProducts.length > visibleCount;
      ...
      <div className="relative flex-1">
        ...
        {needsScroll && canScrollUp && (
          <button
            ...
            className="absolute top-0 left-0 right-0 z-20 flex items-center justify-center h-7 bg-gradient-to-b from-card via-card/90 to-transparent hover:from-muted transition-colors"
        ...
        <div
          ...
          style={{
            maxHeight: needsScroll ? `${containerHeight}px` : undefined,
            scrollbarWidth: 'none',
        ...
        {needsScroll && canScrollDown && (
          <button
            ...
            className="absolute bottom-0 left-0 right-0 z-20 flex items-center justify-center h-7 bg-gradient-to-t from-card via-card/90 to-transparent hover:from-muted transition-colors"
