...
import { Zap, ChevronRight, ShoppingCart, Star, Package } from "lucide-react";
...
export const TopSellingUniversalProducts = ({ limit = 10, title = "টপ বিক্রিত সাধারণ প্রোডাক্ট", subtitle }: TopSellingUniversalProductsProps) => {
  ...
  const { data: topProducts = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data: products } = await supabase.from('universal_products').select('*').eq('is_active', true).order('is_featured', { ascending: false }).order('created_at', { ascending: false }).limit(limit);
  ...
  const getProductLink = (product: any) => `/${product.product_type}/${product.slug}`;
