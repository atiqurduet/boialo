...
export const PublisherBulkActions = ({ onImportComplete }: PublisherBulkActionsProps) => {
  ...
  const handleImport = async () => {
    ...
    for (const row of validRows) {
      try {
        const publisherData = {
          ...
          website_url: row.data.website_url || null,
        ...
        const { error } = await supabase.from('publishers').insert([publisherData]);
        if (error) throw error;
    ...
    setImporting(false);
    if (successCount > 0) {
      toast({ title: 'সফল', description: `${successCount}টি প্রকাশনী ইম্পোর্ট হয়েছে` });
      onImportComplete();
    }
    if (errorCount > 0) {
      toast({ title: 'সতর্কতা', description: `${errorCount}টি প্রকাশনী ইম্পোর্ট করা যায়নি`, variant: 'destructive' });
    }
  };
    ...
    <>
      ...
      <Dialog open={importDialogOpen} onOpenChange={setImportDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh]">
          <DialogHeader>
            <DialogTitle>প্রকাশনী ইম্পোর্ট প্রিভিউ</DialogTitle>
          </DialogHeader>
          ...
          <div className="flex justify-end gap-2 mt-4">
            ...
            <Button onClick={handleImport} disabled={importing || validCount === 0}>
              {importing ? 'ইম্পোর্ট হচ্ছে...' : `${validCount}টি প্রকাশনী ইম্পোর্ট করুন`}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
