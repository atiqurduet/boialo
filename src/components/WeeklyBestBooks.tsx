...
import { Trophy, Star, ChevronRight, ShoppingCart, Calendar } from "lucide-react";
...
interface WeeklyBestBooksProps {
  limit?: number;
  title?: string;
  subtitle?: string;
  useCarousel?: boolean;
...
export const WeeklyBestBooks = ({ limit = 10, title = "সাপ্তাহিক সেরা বই", subtitle, useCarousel = true, columns = 5 }: WeeklyBestBooksProps) => {
  const { addToCart } = useCartContext();
  const weekStart = format(subDays(new Date(), 7), 'yyyy-MM-dd');

  const { data: weeklyBest = [], isLoading } = useQuery({
    queryKey: ['weekly-best-books', limit, weekStart],
    queryFn: async () => {
      ...
      (orderItems || []).forEach(item => { counts[item.product_id] = (counts[item.product_id] || 0) + 1; });

      const sortedIds = Object.entries(counts).sort(([, a], [, b]) => b - a).slice(0, limit).map(([id]) => id);

      if (sortedIds.length === 0) {
        const { data } = await supabase.from('products').select('*').eq('is_active', true).eq('is_featured', true).order('created_at', { ascending: false }).limit(limit);
        return (data || []).map((p, i) => ({ ...p, weekly_sold: 0, rank: i + 1 }));
      }

      const { data: products } = await supabase.from('products').select('*').eq('is_active', true).in('id', sortedIds);
  ...
  const renderCard = (product: any) => (
    <div key={product.id} className="group relative bg-card rounded-xl border overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
      {product.rank <= 3 && (
        <div className="absolute top-2 left-2 z-10 w-8 h-8 bg-gradient-to-br from-yellow-400 to-amber-500 rounded-full flex items-center justify-center text-sm shadow-lg">
    ...
    <section className="my-10">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-gradient-to-br from-amber-500 to-yellow-500 rounded-xl shadow-lg shadow-amber-500/20">
          ...
          <Badge variant="secondary" className="hidden md:flex items-center gap-1 bg-amber-50 text-amber-600 dark:bg-amber-950 dark:text-amber-400 border-0">
